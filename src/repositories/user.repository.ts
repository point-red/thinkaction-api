...
export class UserRepository extends Database {
  ...
  public async findOne4(id: string) {
    return await this.collection.findOne(
      { _id: new ObjectId(id) },
      { projection: { supportingCount: 1, isPublic: 1 } }
    );
  }
  ...
  public async updateUser(id: string, data: Partial<any>) {
    return await this.collection.updateOne(
      { _id: new ObjectId(id) },
      { $set: data }
    );
  ...
  public async updateRemoveHistory(id: string, authUserId: string) {
    return await this.collection.updateOne(
      ...
      { $pull: { historyAccount: new ObjectId(id) as any } }
    );
  }
  ...
  public async updateOne8(id: string, authUserId: string) {
    return await this.collection.updateOne(
      ...
      {
        $push: {
          historyAccount: { $each: [new ObjectId(id)], $position: 0 },
        } as any,
      }
    );
  }
